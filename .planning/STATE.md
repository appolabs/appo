...
**Milestone B delivery (2026-07-04, v4.1.0):** self-serve test-build verbs shipped per `MILESTONE-B-HANDOFF-self-serve-builds.md` (apps-web-app v3.3 Phase 203, AGT-02): `appo build <id> [--platform ios|android]` (test-kind trigger; NOT the removed publish-build verb — kind is server-fixed, publish stays operator-internal), `appo download <id> [--build <n>] [--output <path>]`, `appo devices list|register` (signed 24h enrollment link + QR). New `src/download.mjs` handles the 302→artifact binary path. 222 tests green; commit `689a9ae`.
