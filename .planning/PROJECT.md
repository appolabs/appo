...
Phase 7 complete (v0.2 Ship-Intent Surface — operator boundary, 2026-06-22): the CLI now issues no builds. `appo ship` is create + publish-intent only — `ship --url --name` creates then signals publish-intent via `POST /api/v1/apps/{id}/publish` (reusing `ops.publishApp`), `ship <id>` signals (re)publish-intent; both return immediately and Appo staff issue the build server-side. The client-side build trigger (`ops.triggerBuild`) and the `pollBuild` loop are deleted — no CLI path calls `POST /api/v1/apps/{id}/builds` (the v1 route is 405 / legacy 404 for a user PAT, per `apps-web-app` Phase 189). Build STATUS reads (`appo status <id>`) are retained. `apps update <id>` is rescoped to the three user-editable fields: `--name`/`--url` (→ `PATCH`, metadata silently ignored server-side per Phase 190-01) plus a new `--icon <https image URL>` (→ `ops.setIcon` → `POST /api/v1/apps/{id}/icon`, SSRF-guarded server-side, flat `{icon_url}` response, Phase 190-02); `--meta-name`/`--meta-desc` removed. Gate green (192 tests, lint + typecheck; +negative-assertion doc guards locking the removed flags/transport). The cross-repo backend (`apps-web-app` Phases 189 + 190) shipped 2026-06-21/22. Consumes the operator-boundary model: builds are staff-only, the user/agent expresses intent.

Post-v0.1 surface abstraction (4.0.0, 2026-06-16): the lifecycle surface was narrowed to a single outcome verb. `appo ship` is the only lifecycle verb — `ship --url --name` creates a new app, `ship <id>` rebuilds+republishes an existing one (absorbing the former `reship` and `resubmit`; no first/Nth distinction). The user-facing `build` verb was removed (no `--platform`/`--branch`); the platform is decided server-side. `apps update <id>` merges the former `configure` + `apps set-name` (one verb, one `PATCH /apps/{id}`). `publish` remains as a standalone "publish an already-built app" intent (defaults to the app's stores). CLI↔MCP parity is asserted at the OUTCOME level — `trigger_build` is intentionally MCP-only, `ship` intentionally CLI-only (canonical surface: `apps-web-app/docs/CROSS-SURFACE-PARITY.md` — note that doc predates this `reship`/`resubmit`/`configure` removal and needs re-sync). Gate green (197 tests, lint + typecheck). **Version 4.0.0 is bumped locally but NOT yet published — npm `latest` is still 3.0.0 (the pre-abstraction surface); publishing 4.0.0 makes the abstraction live.**
...
---
*Last updated: 2026-06-22 — Phase 7 complete (v0.2 Ship-Intent Surface): ship = create + publish-intent (no client build/poll), apps update = icon/url/name*
