...
test('configure maps --url and --meta-name to v1 body fields', async () => {
  stubToken();
  installMockFetch({ status: 204 });
  await captureLog(() => run(['configure', '7', '--url', 'https://x', '--meta-name', 'Store Name', ...API]));
  const req = lastRequest();
  expect(req.body).toEqual({ base_url: 'https://x', metadata_name: 'Store Name' });
});

test('configure maps --meta-desc to metadata_description', async () => {
  stubToken();
  installMockFetch({ status: 204 });
  await captureLog(() => run(['configure', '7', '--meta-desc', 'A great app', ...API]));
  const req = lastRequest();
