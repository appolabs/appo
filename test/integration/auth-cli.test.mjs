...
test('logout 204 issues DELETE /user/tokens/current and clears the local token', async () => {
  ...
  expect(lastRequest().path).toMatch(/\/api\/v1\/user\/tokens\/current$/);
...
test('logout 401 (already invalid) still clears the local token + warns, exit 0', async () => {
  ...
  expect(result).toBe(0);
  expect(storedToken('default')).toBe(null); // cleared anyway (finally)
  const out = lines.join('\n');
  expect(out).toMatch(/Could not confirm/i);
  expect(out).not.toMatch(/disk-tok/);
});
...
test('logout --env staging clears only staging, siblings untouched (per-env)', async () => {
  ...
  expect(result).toBe(0);
  expect(storedToken('staging')).toBe(null);
  expect(storedToken('default')).toBe('default-tok'); // sibling untouched
});
...
test('whoami 200 reports env + api_base + app count, never the token', async () => {
  ...
  expect(out).toMatch(/test\.local/);
  expect(out).toMatch(/2 app\(s\)/);
  expect(out).not.toMatch(/disk-tok/);
});
...
test('env list marks the active profile and prints no token', async () => {
  ...
  expect(out).toMatch(/\*\s+staging/); // active marked
  expect(out).not.toMatch(/prod-tok/);
  expect(out).not.toMatch(/stg-tok/);
});
...
test('env list on an empty config prints a hint, exit 0', async () => {
  ...
  expect(result).toBe(0);
  expect(lines.join('\n')).toMatch(/No environments yet.*appo login/i);
});
...
test('--env= (empty value) is a usage error, exit 2', async () => {
  expect(await silentRun(['whoami', '--env='])).toBe(2);
});

test('--api= (empty value) is a usage error, exit 2', async () => {
  expect(await silentRun(['whoami', '--api='])).toBe(2);
});

test('--token= (empty value) is a usage error, exit 2 (no device flow)', async () => {
  expect(await silentRun(['login', '--token='])).toBe(2);
});
...
test('--env staging overrides current for both token and api_base', async () => {
  ...
  expect(req.headers.Authorization).toBe('Bearer stg-tok');
  expect(req.url).toMatch(/stg\.local/);
...
test('--env staging is honored by an ops-routed verb (apps create) — WR-01', async () => {
  ...
  const req = lastRequest();
  expect(req.headers.Authorization).toBe('Bearer stg-tok');   // staging token, not production's
  expect(req.url).toMatch(/stg\.local/);
  expect(req.url).not.toMatch(/prod\.local/);
});
...
test('login --token refuses on 401: stores nothing, single probe, no PAT echo', async () => {
  ...
  expect(result).toBe(1);
  expect(storedToken('default')).toBe(before);
  expect(requests.length).toBe(1); // only the validation probe
  expect(lines.join('\n')).not.toMatch(/bad-pat/);
});
...
test('no auth verb ever prints a stored token (cross-verb sweep)', async () => {
  ...
  for (const tok of ['prod-tok', 'stg-tok', 'good-pat']) {
    expect(out).not.toMatch(new RegExp(tok));
  }
});
