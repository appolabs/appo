...
Create a matching feature module in the SDK so `appo.myfeature.doSomething()` sends the correct message type. See the [SDK architecture](/docs/sdk/architecture) for the message protocol.

**4. Declare permissions:**

If the native API requires platform permissions, add them to `app.json` as described in [Configuration](/docs/native-app/configuration).
